...
import { WorkoutPlannerClient } from './workout-planner-client';
import { redirect } from 'next/navigation';
import Link from 'next/link';

export const metadata = {
  title: 'AI Workout Planner - Fitoholic',
};
    ...
    <>
      <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 animate-fade-up">
        ...
        <div className="flex bg-surface-container-low rounded-[24px] p-1 border border-white/5">
          <Link href="/dashboard/meal-planner" className="px-6 py-2 rounded-[20px] text-on-surface-variant font-label-caps text-[10px] uppercase font-bold tracking-widest hover:text-on-surface transition-colors">
            Meal Plan
          </Link>
          <Link href="/dashboard/workout-planner" className="px-6 py-2 rounded-[20px] bg-surface-variant text-primary font-label-caps text-[10px] uppercase font-bold tracking-widest shadow-sm">
            Workout Split
          </Link>
        </div>
      </header>
      ...
      {!isPremium ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center glass-card rounded-[24px] max-w-2xl mx-auto w-full animate-fade-up delay-100">
          ...
          <p className="text-on-surface-variant mb-8 text-lg">
            Upgrade to Premium to get AI-generated, personalized 7-day workout plans to crush your fitness goals.
          </p>
          <Link 
